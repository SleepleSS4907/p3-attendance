...
if st.button("🚀 อัปเดตข้อมูลลงชีต ป.3 'เช็คชื่อรายวัน'", type="primary", use_container_width=True):
    ...
    if sheet:
        try:
            ...
            if col_index:
                # แก้ไขระบบการกรอกข้อมูล: ดึงเซลล์เป้าหมายออกมาก่อนเป็นชุด
                # เลขที่ 1 อยู่แถว 2, เลขที่ 12 อยู่แถว 13
                cell_list = sheet.range(2, col_index, 13, col_index)
                
                # นำรหัสสถานะ (1, ข, ป, ล) ใส่เข้าไปในวัตถุเซลล์แต่ละช่อง
                for i, code_val in enumerate(final_upload_values):
                    cell_list[i].value = code_val
                
                # ยิงอัปเดตชุดข้อมูลขึ้นกูเกิลชีตทีเดียวพร้อมกัน ป้องกัน ValueError บิ๊กตู้ม
