...
st.info(f"🔍 ระบบกำลังล็อกเป้าหมายค้นหาคอลัมน์วันที่: **{display_date_style}** (หรือรูปแบบที่ใกล้เคียง)")
...
if st.button("🚀 อัปเดตข้อมูลลงชีต ป.3 'เช็คชื่อรายวัน'", type="primary", use_container_width=True):
    ...
    if sheet:
        with st.spinner("กำลังบันทึกข้อมูลลง Google Sheets..."):
            try:
                # ดึงข้อมูลแถวที่ 1 ทั้งหมดมาค้นหาคอลัมน์วันที่
                header_row = sheet.row_values(1)
                col_index = None
                
                # ปรับโครงสร้าง Regex ให้ยืดหยุ่นและแม่นยำขึ้น รองรับเลข 0 นำหน้าและไม่นำหน้า
                ...
                if col_index:
                    # ปรับปรุง: ใช้พิกัดในการระบุเพื่อป้องกันข้อผิดพลาดของ gspread
                    # ดึงช่วงข้อมูลแถวที่ 2 ถึงแถวที่ 13 ตามคอลัมน์ที่เจอ
                    cell_list = sheet.range(start_row=2, start_column=col_index, end_row=13, end_column=col_index)
                    
                    # นำรหัสสถานะใส่เข้าไปในแต่ละช่องเซล์ที่ดึงมา
                    for i, code_val in enumerate(final_upload_values):
                        cell_list[i].value = code_val
                    
                    # ยิงชุดข้อมูลขึ้นกูเกิลชีตทีเดียวพร้อมกันแบบปลอดภัย
        ...
        st.warning("⚠️ ไม่สามารถเชื่อมต่อ Google Sheets ได้ กรุณาตรวจสอบไฟล์สิทธิ์เข้าถึง (Credentials)")
